...
#define LOG_DOMAIN sof_dai_ssp_mn
					    ...
					    uint32_t mclk_rate)
{
	...
	for (i = 0; i <= MAX_SSP_FREQ_INDEX; i++) {
		if (ssp_freq_z[i].freq % mclk_rate == 0) {
			clk_index = i;
			break;
		}
	}
	...
	/* select source clock */
	mdivc |= MCDSS(ssp_freq_sources_z[clk_index]);
...
static inline int check_current_mclk_source(uint16_t mclk_id, uint32_t mclk_rate)
{
	...
	if (ssp_freq_z[mn->mclk_source_clock].freq % mclk_rate != 0) {
...
int z_mn_set_mclk(uint16_t mclk_id, uint32_t mclk_rate)
{
	...
	ret = set_mclk_divider(mclk_id,
			       ssp_freq_z[mn->mclk_source_clock].freq /
			       mclk_rate);
	if (!ret)
			    ...
			    uint32_t *scr_div, uint32_t *m, uint32_t *n)
{
	...
	if (is_mclk_source_in_use()) {
		if (find_mn(ssp_freq_z[mn->mclk_source_clock].freq, bclk, scr_div, m, n))
			return mn->mclk_source_clock;

		LOG_WRN("BCLK %d warning: cannot use MCLK source %d",
			bclk, ssp_freq_z[mn->mclk_source_clock].freq);
	}

	/* searching the smallest possible bclk source */
	for (i = 0; i <= MAX_SSP_FREQ_INDEX; i++)
		if (ssp_freq_z[i].freq % bclk == 0) {
			*scr_div = ssp_freq_z[i].freq / bclk;
			return i;
		}

	/* check if we can get target BCLK with M/N */
	for (i = 0; i <= MAX_SSP_FREQ_INDEX; i++)
		if (find_mn(ssp_freq_z[i].freq, bclk,
			    scr_div, m, n))
			return i;
...
static int find_clk_ssp_index(uint32_t src_enc)
{
	...
	for (i = 0; i <= MAX_SSP_FREQ_INDEX; i++)
		if (ssp_freq_sources_z[i] == src_enc)
			return i;
					       ...
					       uint32_t *m, uint32_t *n)
{
	...
	/* select source clock */
	mdivc |= MNDSS(ssp_freq_sources_z[clk_index]);
					       ...
					       uint32_t *m, uint32_t *n)
{
	...
	if (find_mn(ssp_freq_z[mn->bclk_source_mn_clock].freq, bclk, scr_div, m,
		    n))
		goto out;
