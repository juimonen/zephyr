...
#ifndef __DAI_PARAMS_INTEL_H__
#define __DAI_PARAMS_INTEL_H__
...
/* HDA Configuration Request - SOF_DAI_HDA_CONFIG */
struct sof_dai_hda_params {
	uint32_t reserved0;
...
/* ALH Configuration Request - SOF_DAI_ALH_CONFIG */
struct sof_dai_alh_params {
	uint32_t reserved0;
	uint32_t stream_id;
	uint32_t rate;
	uint32_t channels;
...
/* DMIC Configuration Request - SOF_DAI_DMIC_CONFIG */
...
struct sof_dai_dmic_params {
	...
	struct sof_dai_dmic_pdm_ctrl pdm[SOF_DAI_INTEL_DMIC_NUM_CTRL];
} __attribute__((packed, aligned(4)));

#endif /* __DAI_PARAMS_INTEL_H__ */
